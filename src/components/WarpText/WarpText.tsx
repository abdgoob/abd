...
const drawLine = (
  ...
  y: number,
  letterSpacing: number,
  textAlign: RuntimeProps['textAlign']
): void => {
  const chars = Array.from(line);
  const lineWidth = measureLine(ctx, line, letterSpacing);
  let cursor = textAlign === 'center' ? x - lineWidth / 2 : textAlign === 'right' ? x - lineWidth : x;
...
const buildTextCanvas = ({
  ...
  dpr,
  props
}: BuildTextCanvasArgs): { canvas: HTMLCanvasElement; contentHeight: number } => {
  const canvas = document.createElement('canvas');
  canvas.width = Math.max(1, Math.floor(width * dpr));
  canvas.height = Math.max(1, Math.floor(height * dpr));

  const ctx = canvas.getContext('2d');
  if (!ctx) return { canvas, contentHeight: height };
  ...
  const fontSizePx = parseFloat(computed.fontSize) || 96;
  const fontFamily = computed.fontFamily || 'sans-serif';
  const fontWeight = computed.fontWeight || String(props.fontWeight);
  const textColor = computed.color || props.color;
  const letterSpacing = computed.letterSpacing === 'normal' ? 0 : parseFloat(computed.letterSpacing) || 0;
  ...
  applyFont();
  const edgeInset = Math.max(2, fontSizePx * 0.04);
  lines.splice(
    0,
    lines.length,
    ...wrapText(ctx, props.text, Math.max(width - edgeInset * 2, 1), letterSpacing)
  );

  const blockHeight = lineHeight * lines.length;
  const verticalInset = Math.max(2, fontSizePx * 0.08);
  const contentHeight = Math.ceil(blockHeight + verticalInset * 2);
  const startY = verticalInset + lineHeight / 2;
  const x =
  ...
  lines.forEach((line, index) =>
    drawLine(ctx, line, x, startY + index * lineHeight, letterSpacing, props.textAlign)
  );
...
const WarpText = ({
  ...
  const onLayoutHeightRef = useRef(onLayoutHeight);

  useEffect(() => {
    onReadyRef.current = onReady;
    onLayoutHeightRef.current = onLayoutHeight;
  }, [onReady, onLayoutHeight]);
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      const { canvas: textCanvas, contentHeight } = buildTextCanvas({
        container,
        width: rect.width,
        height: rect.height,
        dpr,
      ...
      renderOnce();
      onLayoutHeightRef.current?.(contentHeight);
      if (contentHeight > rect.height + 1) return;
      if (!ready) {
        ready = true;
        onReadyRef.current?.();
      }
